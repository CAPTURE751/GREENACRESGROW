...
export default function BudgetSimulator() {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("venture-ai-advisor", {
        body: {
          ...
          profitPerAcre: revenue.profitPerAcre,
          breakEvenPrice: revenue.breakEvenPrice,
          yieldPerAcre: inputs.expectedYieldPerAcre,
          marketPrice: inputs.marketPricePerUnit,
        },
      });
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      toast({ title: "PDF Downloaded", description: "Venture budget report saved." });
    } catch (e: any) {
  ...
  const numField = (label: string, field: keyof VentureInputs, placeholder?: string) => (
    <div className="space-y-1">
      ...
      <Input
        ...
        min={0}
        value={inputs[field] || ""}
        onChange={(e) => numSet(field, e.target.value)}
        ...
        <Tabs defaultValue="inputs" className="space-y-6">
          ...
          <TabsContent value="sensitivity" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={[...sensitivity.scenarios.map((s) => ({ name: s.scenario, profit: s.profit, type: "yield" })), ...sensitivity.priceScenarios.map((s) => ({ name: s.scenario, profit: s.profit, type: "price" }))]}>
                    <CartesianGrid strokeDasharray="3 3" />
